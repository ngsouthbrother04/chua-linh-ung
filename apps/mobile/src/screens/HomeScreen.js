...
export default function HomeScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    const pingPresence = async () => {
      ...
      try {
        ...
        console.log("Không gửi được heartbeat:", error);
      }
    };

    pingPresence();
    heartbeatInterval = setInterval(pingPresence, 30000);
      ...
      if (lang.code === "vi") {
        ...
        try {
          ...
          const translatedValues = await Promise.all(
            Object.values(uiLabels).map((val) =>
              autoTranslate(val.replace(/🔊 |⏹ /g, ""), lang.code),
            ),
          );
          const newLabels = {};
          keys.forEach((key, i) => {
  ...
  const handleAutoTranslate = async (poi, shouldPlay = false) => {
    if (!poi) return;
    const originName =
      typeof poi.name === "object"
        ? poi.name?.vi || poi.name?.en || ""
        : poi.name || "";
    const originDesc =
      typeof poi.description === "object"
        ? poi.description?.vi || poi.description?.en || ""
        : poi.description || "";
  ...
  const handleBarCodeScanned = async ({ data }) => {
    ...
    const foundPoi = poisRef.current.find(
      (p) => p.id.toString() === data.trim(),
    );
    if (foundPoi) {
      ...
      mapRef.current?.animateToRegion(
        {
          latitude: parseFloat(foundPoi.latitude),
          longitude: parseFloat(foundPoi.longitude),
          latitudeDelta: 0.005,
  ...
  const checkGeofencing = async () => {
    ...
    if (nearestPoi) {
      if (lastPlayedPoiId.current !== nearestPoi.id) {
        ...
        const nameStr =
          typeof nearestPoi.name === "object"
            ? nearestPoi.name?.vi || nearestPoi.name?.en || ""
            : nearestPoi.name;
        ...
        {pois.map((p) => (
          <React.Fragment key={p.id}>
            <Circle
              ...
              radius={100}
              strokeColor="rgba(255, 111, 0, 0.4)"
            ...
            >
              <View
                style={[
                  styles.customMarker,
                  selectedPoi?.id === p.id && styles.activeMarker,
                ]}
              >
                <Ionicons
                  name="restaurant"
                  size={14}
                  color={selectedPoi?.id === p.id ? "#fff" : "#FF6F00"}
                />
              </View>
            </Marker>
          </React.Fragment>
        ...
        <ScrollView contentContainerStyle={{ padding: 20 }}>
          <View style={styles.infoTopRow}>
            <View style={styles.badge}>
              <Text style={styles.badgeText}>{selectedPoi?.type || "POI"}</Text>
            </View>
            {selectedPoi && (
              <Text style={styles.distanceText}>
                {getDistance(
                  ...
                  parseFloat(selectedPoi.latitude),
                  parseFloat(selectedPoi.longitude),
                ).toFixed(0)}
                m
              </Text>
            )}
          ...
          {isTranslating ? (
            ...
            <Text style={styles.poiDesc}>
              {translatedData.desc ||
                "Chọn một địa điểm trên bản đồ để khám phá câu chuyện và nghe thuyết minh tự động."}
            </Text>
          )}
        ...
        <View style={styles.menuOverlay}>
          <View style={styles.menuSideBar}>
            ...
            <View style={styles.menuBody}>
              ...
              >
                <Text style={[styles.menuItemIcon, { fontSize: 20 }]}>
                  {lang.flag}
                </Text>
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <TouchableOpacity
                    style={[
                      styles.langGridItem,
                      lang.code === item.code && styles.langGridItemActive,
                    ]}
                    onPress={() => {
                      setLang(item);
