...
| --- | -------- | ---------------------------- | ------------------------------------------------------------- |
| UC1 | AC-UC1   | TC-1.1                       | TC-1.2 -> TC-1.5, TC-15.1, TC-15.2, TC-18.1, TC-18.2, TC-18.3 |
...
| UC7 | AC-UC7   | TC-7.1                       | TC-7.2, TC-7.3, TC-7.4, TC-18.7                               |
| UC8 | AC-UC8   | TC-8.1                       | TC-8.2, TC-8.3, TC-9.x, TC-10.x, TC-15.4, TC-18.4, TC-18.5    |
...
### Scenario 18 – API Contract Completeness (Missing APIs Risk Closure)

#### TC-18.1: Token Refresh Success
...
#### TC-18.3: Logout Contract

| Field               | Value                                                                 |
| ------------------- | --------------------------------------------------------------------- |
| **Test Case ID**    | TC-18.3                                                               |
| **Title**           | Logout invalidates active session contract                            |
| **Steps**           | 1. Call `POST /api/v1/auth/logout` with valid access token            |
| **Expected Result** | ✅ Logout success response; subsequent protected call is unauthorized |
| **Status**          | ⬜ Not Run                                                            |

#### TC-18.4: Incremental Sync Success

| Field               | Value                                                                    |
| ------------------- | ------------------------------------------------------------------------ |
| **Test Case ID**    | TC-18.4                                                                  |
| **Title**           | Delta sync applies changes from `fromVersion` to latest                  |
| **Precondition**    | Client at older version with delta window still supported                |
| **Steps**           | 1. Call `POST /api/v1/sync/incremental` with `fromVersion`               |
| **Expected Result** | ✅ Returns changed POIs/tours + deleted IDs and `requiresFullSync=false` |
| **Status**          | ⬜ Not Run                                                               |

#### TC-18.5: Incremental Sync Fallback

| Field               | Value                                                                                               |
| ------------------- | --------------------------------------------------------------------------------------------------- |
| **Test Case ID**    | TC-18.5                                                                                             |
| **Title**           | Delta window exceeded triggers full sync fallback                                                   |
| **Precondition**    | Client version outside server delta window                                                          |
| **Steps**           | 1. Call `POST /api/v1/sync/incremental` with old `fromVersion`                                      |
| **Expected Result** | ⚠️ Returns `requiresFullSync=true` (or equivalent) and client falls back to `GET /api/v1/sync/full` |
| **Status**          | ⬜ Not Run                                                                                          |

#### TC-18.6: Public POI API Contract

| Field               | Value                                                                                                       |
| ------------------- | ----------------------------------------------------------------------------------------------------------- |
| **Test Case ID**    | TC-18.6                                                                                                     |
| **Title**           | Public POI endpoints return stable payload shape                                                            |
| **Steps**           | 1. Call `GET /api/v1/pois` 2. Call `GET /api/v1/pois/:id` 3. Call `POST /api/v1/pois/search/radius`         |
| **Expected Result** | ✅ Responses include expected fields, pagination/meta, and valid error code for not-found/invalid geo input |
| **Status**          | ⬜ Not Run                                                                                                  |

#### TC-18.7: Public Tour API Contract

| Field               | Value                                                                                 |
| ------------------- | ------------------------------------------------------------------------------------- |
| **Test Case ID**    | TC-18.7                                                                               |
| **Title**           | Public tour endpoints return ordered data contract                                    |
| **Steps**           | 1. Call `GET /api/v1/tours` 2. Call `GET /api/v1/tours/:id`                           |
| **Expected Result** | ✅ Tour detail includes ordered `poiIds`; missing tour returns stable not-found error |
| **Status**          | ⬜ Not Run                                                                            |

#### TC-18.8: Presence Heartbeat Contract

| Field               | Value                                                                    |
| ------------------- | ------------------------------------------------------------------------ |
| **Test Case ID**    | TC-18.8                                                                  |
| **Title**           | Presence heartbeat endpoint returns canonical time windows               |
| **Steps**           | 1. Call `POST /api/v1/analytics/presence/heartbeat`                      |
| **Expected Result** | ✅ Response includes `onlineNowWindowSec=90` and `active5mWindowSec=300` |
| **Status**          | ⬜ Not Run                                                               |
...
- [ ] Missing API risk closure (TC-18.1 through TC-18.8)
